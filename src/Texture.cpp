	...
	: m_TextureID(0), m_FilePath(path), m_Data(nullptr), m_Width(0), m_Height(0), m_nrChannels(0) //You can initialize here
{
	...
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR_MIPMAP_LINEAR);//what to do when we shrink it
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);//what to do when we enlarge it
	...
	{
		std::cout << "Texture not here :c" << std::endl;
	}
